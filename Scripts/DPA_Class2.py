...
def get_all_permutations(conditions):
    ...
    comb_list = list(permutations(conditions, 2))
    
    print("\nAvailable condition permutations:")
    for i, (cond1, cond2) in enumerate(comb_list, 1):
    ...
    while True:
        try:
            ...
            if any(idx < 1 or idx > len(permutations) for idx in selected_indices):
                raise ValueError("Invalid permutation number(s). Please try again.")
...
def perform_analysis(matrix_data, cond1, cond2, is_upregulated, paired=False):
    """
    Perform two-class analysis (with replicates)
    Class 2 uses cl=0 for first condition and cl=1 for second condition
    """
    ...
    # Print analysis info
    print(f"\nPerforming Class 2 Analysis (two-class, {'paired' if paired else 'unpaired'})")
    ...
    # Create class labels for two-class analysis
    # Class 0 for first condition, Class 1 for second condition
    cl = robjects.IntVector([0] * len(cond1_cols) + [1] * len(cond2_cols))
    
    print(f"Class vector: {list(cl)}")
    
    # Prepare arguments for RankProducts function
    rankprod_args = {
        "data": r_matrix,
        "cl": cl,
        "logged": True,
    ...
    # Conditionally add paired argument if needed
    if paired:
        rankprod_args["paired"] = robjects.BoolVector([paired])
    
    # Perform RankProd analysis
    result = rankprod.RankProducts(**rankprod_args)
    
    # Process results 
    paired_str = "_paired" if paired else ""
    filename = f"{cond1}_vs_{cond2}{paired_str}_results.csv"
...
def check_if_paired_possible(matrix_data, conditions):
    """
    Check if paired analysis is possible by ensuring each condition has the same number of replicates
    with matching replicate numbers
    """
    paired_permutations = []
    
    # Get all possible permutations of conditions
    all_permutations = list(permutations(conditions, 2))
    
    for cond1, cond2 in all_permutations:
        # Get replicate numbers for each condition
        cond1_cols = [col for col in matrix_data.columns if cond1 in col]
        cond2_cols = [col for col in matrix_data.columns if cond2 in col]
        
        cond1_reps = [col.split('_Rep')[-1] if '_Rep' in col else '' for col in cond1_cols]
        cond2_reps = [col.split('_Rep')[-1] if '_Rep' in col else '' for col in cond2_cols]
        
        # Check if conditions have the same number of replicates and matching replicate numbers
        if len(cond1_reps) == len(cond2_reps) and len(cond1_reps) > 1:
            common_reps = set(cond1_reps) & set(cond2_reps)
            if len(common_reps) == len(cond1_reps):
                paired_permutations.append((cond1, cond2))
...
def main():
    ...
    try:
        ...
        user_input = input("Enter prioritization method ('higher' or 'lower', default is 'higher'): ").lower()
        if user_input == 'lower':
